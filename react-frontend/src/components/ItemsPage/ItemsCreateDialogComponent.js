...
const ItemsCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.itemName)) {
                error["itemName"] = `Item name field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            itemName: _entity?.itemName,QuantityItem: _entity?.QuantityItem,
    ...
    return (
        <Dialog header="Create Items" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="itemName" className="w-full mb-3 p-inputtext-sm" value={_entity?.itemName} onChange={(e) => setValByKey("itemName", e.target.value)}  required  />
            </span>
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="QuantityItem">Quantity item:</label>
                <InputNumber id="QuantityItem" className="w-full mb-3 p-inputtext-sm" value={_entity?.QuantityItem} onChange={(e) => setValByKey("QuantityItem", e.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["QuantityItem"]) ? (
              <p className="m-0" key="error-QuantityItem">
                {error["QuantityItem"]}
              </p>
            ) : null}
          </small>
